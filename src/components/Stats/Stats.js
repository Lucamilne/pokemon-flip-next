...
export default function Stats({ stats, originalStats }) {
    ...
    return (
        <div className="absolute top-0 left-0 bg-linear-to-b from-black/15 to-transparent pl-0.5 md:pl-1 w-full flex">
            <div className="mt-0.5 font-mono text-[10px] md:text-xs z-20 font-bold leading-none text-shadow-sm/50 flex flex-col justify-between text-center">
                <div className={conditionalClass(1)}>{formattedStats[1]}</div>
                <div className="flex justify-between -my-0.5 md:m-0 gap-3 md:gap-5">
